...
# Ensure CSP forwarding before for YNH core versions earlier than v13.0.8
# Cf. https://github.com/YunoHost/issues/issues/2301
fix_csp_forwarding() {
    CSP_fix_ynh_core_version="13.0.8" #version number formatted as such "13.0.6+202708201400", it works fine with additional subversion level and/or without the +{DATE}
    current_ynh_core_version="$(yunohost --version | grep "yunohost:" --after-context 2 | tail -n 1 | sed -e 's/^\s*version: //')"
    if dpkg --compare-versions "$current_ynh_core_version" ge "$CSP_fix_ynh_core_version" && grep -q '# Force usage of some app-defined headers' "/etc/nginx/conf.d/$domain.d/$app.conf"; then
        # Remove pre-Trixie fix from nginx.conf
	    sed -i '/\s*\# Force usage of/,/content_security_policy";/d' "/etc/nginx/conf.d/$domain.d/$app.conf"
